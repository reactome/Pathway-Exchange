...
/**
 * Export PSIMI from Reactome.
 * 
 * Provides a field-based interface to this functionality, so that it can be
 * incorporated into the server end of an advanced PSIMI generator.
 * 
 * To use, call the addField method as many times as you need to build
 * up the parameter settings for the underlying PSIMIBuilder object.
 ...
 *
 */
public class PSIMIBuilderFields extends PSIMIBuilder {
	/**
		...
		if (lowerCaseFieldName.equals("host")) {
			...
			if (queryLines.size() > 0) {
				for (String queryLine: queryLines)
					try {
						...
						System.err.println("PSIMIBuilderFields.addField: WARNING - problem extracting a valid reaction DB_ID from string: " + queryLine);
			...
			if (queryLines.size() > 0) {
				...
				try {
					...
					System.err.println("PSIMIBuilderFields.addField: WARNING - problem extracting a valid pathway DB_ID from string: " + pathwayDbIdString);
					e.printStackTrace(System.err);
				}
			}
		} else if (lowerCaseFieldName.equals("sp")) {
			if (queryLines.size() > 0) {
				String speciesString = queryLines.get(0);
				try {
					getPathwayReactionHashHandler().addSpeciesFilter(speciesString);
				} catch (NumberFormatException e) {
					System.err.println("PSIMIBuilderFields.addField: WARNING - problem extracting a species name/DB_ID from string: " + speciesString);
					e.printStackTrace(System.err);
				}
			}
		} else if (lowerCaseFieldName.equals("filter")) {
			if (filterQueryLineParser(queryLines).size() > 0)
				System.err.println("PSIMIBuilderFields.addField: WARNING - extra terms in the filter specification");				
		} else
			System.err.println("PSIMIBuilderFields.addField: WARNING - unknown field name " + lowerCaseFieldName);
	}
	...
	private List<String> filterQueryLineParser(List<String> queryLines) {
		...
		if (newQueryLines.size() < 4) {
			System.err.println("PSIMIBuilderFields.addField: WARNING - not enough elements left in newQueryLines to build a filter, will not continue parsing.");
			System.err.print("PSIMIBuilderFields.addField: newQueryLines=[");
		...
		if (filterType.equals("inc"))
			...
			System.err.println("PSIMIBuilderFields.addField: WARNING - unknown filter type " + filterType + ", will not continue parsing.");
		...
		try {
			if (dbAdaptor.getSchema().isValidClass(trueInstanceClassName)) {
				...
				if (!foundTrueInstanceClassName) {
					System.err.println("PSIMIBuilderFields.getTrueInstanceClassName: WARNING - could not find an instance class matching " + trueInstanceClassName);
					return null;
				}
			}
		} catch (Exception e) {
			System.err.println("PSIMIBuilderFields.getTrueInstanceClassName: WARNING - problem with instanceClassName=" + trueInstanceClassName);
			e.printStackTrace(System.err);
			return null;
		}
	...
	static public void main(String[] args) {
		PSIMIBuilderFields builder = new PSIMIBuilderFields();
		...
		for (int i=0; i<args.length; i++) {
			...
			if (s.matches("^[a-zA-Z_]+=")) {
				...
				builder.addField(fieldName, builder.parseQueryLines(fieldValue));
			}
			else if (s.equals("-o")) {
				i++;
				if (i<args.length)
		...
		System.err.println("PSIMIBuilderFields.main: sending document");

		Dumper.dumpToFile(builder.getEntrySet(), outputFile);
		
		System.err.println("PSIMIBuilderFields.main: done");
	}

	private void printHelp() {
		System.out.println("Usage:");
		System.out.println("");
		System.out.println("java org.reactome.sbml.PSIMIBuilderFields <options>");
		System.out.println("");
